-- MageHUDRingTemplate_ Ring Template
----------------------------------------------------------------------
--
...
setSliceFuncs[1] = function(tex)
	tex:SetTexCoord(0, 1, 1, 0)
end
setSliceFuncs[2] = function(tex)
	tex:SetTexCoord(1, 0, 1, 0)
end
setSliceFuncs[3] = function(tex)
	tex:SetTexCoord(1, 0, 0, 1)
end
setSliceFuncs[4] = function(tex)
...
function MageHUDRingTemplate:DoQuadrantReversed(A, T, SS)
...
function MageHUDRingTemplate:DoQuadrant(A, T, SS)
	-- Grab local references to important textures
	local C = self.chip
	local S = self.slice
...
function MageHUDRingTemplate:SetAngle(angle)
...
function MageHUDRingTemplate:CallTextureMethod(method, ...)
...
function MageHUDRingTemplate:SetRingTextures(ringFactor,
                                            ringTexture,
                                            sliceTexture)
...
function MageHUDRingTemplate:SetReversed(isReversed)
	if (isReversed) then
		isReversed = true
	else
		isReversed = nil
...
function MageHUDRingTemplate:SetMax(max)
	if max == nil then
		max = 1
	end
	self.maxValue = max
...
function MageHUDRingTemplate:SetValue(value)
	if value == nil then
		value = 0
	end
	if value == 0 then
...
function MageHUDRingTemplate:DoFadeUpdate(tdelta)
	tdelta = arg1
...
function MageHUDRingTemplate:AlphaUpdate()
	--[[
...
function MageHUDRingTemplate:SetRingAlpha(destAlpha, instant)
...
function MageHUDRingTemplate:UpdateColor(color)
	if color == nil then
...
function MageHUDRingTemplate:UpdateAlpha()

	local isInCombat = false

	if (string.find(self.name, "Party")) then
	...
	if (self.f.pulse) then
		...
		if (self.parent.db.profile.RingVisibility == 1 or self.parent.db.profile.RingVisibility == 3) then
			...
			if (self.parent.db.profile.RingVisibility == 3 and isInCombat) then
				...
				if (string.find(self.name, "Mana") and UnitPowerType(self.unit or self:GetParent().unit or "player") == 1 and self.f.maxValue > 0) then
...
function MageHUDRingTemplate:GhostMode(state, unit)
	local color = { ["r"] = 0.75, ["g"] = 0.75, ["b"] = 0.75 }
	local fh, fm
	if (unit == "player") then
		fh, fm = MageHUD:GetModule("Health"), MageHUD:GetModule("Mana")
	end
	if (state) then
		-- Prepare health ring
...
-- get things going
function MageHUDRingTemplate:OnLoad(frame)
...
function MageHUDRingTemplate:OnLoadBG(frame)
	-- Grab texture references and frame name
	--frame.name = frame
	--	frame.name = frame
	if (not frame.quadrants) then
